...
class Monster(object):
  def __init__(self, name, image, hp, atk, dfe, spe, spd, crR, crF, types, moves):
    ...
    if isinstance(image, str):
      self.image : pygame.Surface = pygame.image.load("Assets/Monsters/Images/" + image)
    else:
      self.image : pygame.Surface = image
    self.hp : int = hp
    self.attack : int = atk
    self.defense : int = dfe
    self.special : int = spe
    self.speed : int = spd
    self.critRate : float = crR
    self.critFactor : int = crF
